...
const AvatarSelect: React.FC<AvatarSelectProps> = ({ selectedId, onSelect, onConfirm }) => {
  ...
  return (
    <div className="relative my-auto flex h-full max-h-[850px] w-full max-w-6xl flex-col items-center gap-3 px-3 py-6 md:gap-8 md:px-8 md:py-8 min-h-0">
      ...
      <div className="glass-panel relative z-10 flex w-full max-w-2xl items-center gap-2 overflow-hidden rounded-[1.25rem] px-3 py-3 shrink-0 md:rounded-[1.5rem] md:gap-3 md:px-6 md:py-5">
        <div className={`relative flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-white/20 ${selectedAvatar.color} text-3xl shadow-xl md:h-24 md:w-24 md:text-5xl md:rounded-2xl`}>
          <div className="shine" />
        ...
        <div className="min-w-0 flex-1 text-left text-white">
          ...
          <p className="hidden md:block mt-0.5 max-w-lg text-[11px] font-bold leading-tight text-white/70 sm:text-sm md:text-base">
      ...
      <div className="shrink-0 pt-2 w-full max-w-sm">
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={onConfirm}
          className="game-button-primary relative z-10 w-full rounded-[1.25rem] px-8 py-3 text-lg md:rounded-[1.5rem] md:px-20 md:py-6 md:text-3xl"
        >
          Let&apos;s Go
        </motion.button>
      </div>
