...
const GameActionDock: React.FC<GameActionDockProps> = ({
  ...
  return (
    <div className="game-shell-zone game-shell-zone-actions mt-0.5 flex shrink-0 items-center justify-center md:mt-2">
      <div className={`ui-panel-unified fantasy-dock-shell aaa-dock-shell flex items-center ${resolvedCompact ? 'gap-1.5 px-1.5 py-1 md:gap-2 md:px-2 md:py-1.5' : 'gap-2 px-2 py-1.5 md:gap-3 md:px-3 md:py-2'}`}>
        <button
          onClick={() => {
            triggerHaptic('tap');
            onBack();
        ...
        <button
          onClick={toggleMute}
          className={`ui-icon-button game-dock-button aaa-dock-button ${resolvedCompact ? 'aaa-dock-button-compact' : ''} ${resolvedAccentClass} ${mutedState ? 'ring-2 ring-white/60 ring-offset-2 ring-offset-transparent' : ''}`}
