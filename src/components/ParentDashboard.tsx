...
const ParentDashboard: React.FC<ParentDashboardProps> = ({ player, onBack }) => {
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden licensed-shell-bg">
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col gap-2.5 px-4 pb-24 md:gap-4 md:px-8 md:pb-8">
        <div className="grid grid-cols-4 gap-2 md:gap-4">
          {stats.map(item => (
            <div
              key={item.label}
              className="rounded-[1rem] border border-white/12 bg-black/25 p-2.5 text-center text-white shadow-xl backdrop-blur-xl md:rounded-[1.75rem] md:p-4"
            >
        ...
        <div className="grid min-h-0 flex-1 gap-2.5 md:grid-cols-[1.18fr_0.92fr] md:gap-4">
          <section className="flex min-h-0 flex-col rounded-[1.4rem] border border-white/12 bg-black/25 p-3 text-white shadow-xl backdrop-blur-xl md:rounded-[2rem] md:p-5">
            ...
            <div className="mt-2 grid min-h-0 grid-cols-2 gap-2 md:mt-4 md:gap-3">
              {report.islandReports.map(area => {
                ...
                return (
                  <div
                    key={area.id}
                    className={`flex min-h-0 flex-col rounded-[1rem] border border-white/10 bg-white/8 p-2.5 md:rounded-[1.4rem] md:p-3 ${tone.glow}`}
                  >
          ...
          <section className="grid min-h-0 gap-2.5 md:grid-rows-[auto_auto_1fr] md:gap-4">
            <div className="rounded-[1.4rem] border border-white/12 bg-black/25 p-3 text-white shadow-xl backdrop-blur-xl md:rounded-[2rem] md:p-5">
            ...
            <div className="rounded-[1.4rem] border border-white/12 bg-black/25 p-3 text-white shadow-xl backdrop-blur-xl md:rounded-[2rem] md:p-5">
              ...
              <div className="mt-2 rounded-[1rem] border border-white/10 bg-white/8 px-3 py-2.5 md:mt-3 md:rounded-[1.4rem] md:px-4 md:py-3.5">
                <p className="text-[10px] leading-snug text-white/76 md:text-sm">
                  {coachingNote}
                </p>
              </div>
