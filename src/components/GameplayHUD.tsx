  ...
  return (
    <div className="w-full flex shrink-0 flex-col gap-1.5 md:gap-3">
      <div className={`w-full rounded-[1.4rem] border-2 md:rounded-[2rem] md:border-4 ${accentBorder} bg-white/70 shadow-[0_18px_50px_rgba(15,23,42,0.16)] overflow-hidden relative`}>
        ...
        <div className="relative flex flex-col gap-2 p-2.5 md:p-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-2.5 md:gap-4">
            <motion.div
              whileHover={{ scale: 1.06, rotate: -4 }}
              className={`${avatar?.color || 'bg-slate-200'} h-10 w-10 md:h-20 md:w-20 rounded-[1.1rem] md:rounded-[2rem] flex items-center justify-center border-2 border-white/70 relative overflow-hidden shrink-0 bg-center bg-cover`}
            >
            ...
            <div className="min-w-0">
              <div className="text-[10px] md:text-xs font-black uppercase tracking-[0.24em] text-slate-500">Gameplay</div>
              <h2 className={`text-lg md:text-3xl font-black tracking-tight ${accentText} truncate`}>{title}</h2>
              <div className={`mt-0.5 inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-[9px] md:mt-1 md:gap-2 md:px-3 md:text-sm font-bold ${accentBorder} ${accentSoftBg} ${accentText}`}>
          ...
          <div className="grid w-full grid-cols-3 gap-1.5 md:gap-3 lg:min-w-[360px]">
            {statLabel && (
              <div className={`rounded-xl md:rounded-[1.5rem] border-2 ${accentBorder} ${accentSoftBg} px-1.5 py-1.5 md:px-3 md:py-3 text-center`}>
            ...
            <div className={`rounded-xl md:rounded-[1.5rem] border-2 ${accentBorder} bg-white/78 px-1.5 py-1.5 md:px-3 md:py-3 text-center`}>
      ...
      <div className={`w-full rounded-full border-2 ${accentBorder} bg-white/78 p-1 shadow-[inset_0_2px_8px_rgba(15,23,42,0.08)]`}>
        <div className="relative h-3.5 md:h-6 overflow-hidden rounded-full bg-slate-200/60">
