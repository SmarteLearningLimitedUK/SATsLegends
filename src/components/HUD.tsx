...
const HUD: React.FC<HUDProps> = ({ score, targetScore, timeLeft, level, avatar }) => {
  ...
  return (
    <div className="w-full max-w-2xl px-2 md:px-4 py-1 flex flex-col gap-2 md:gap-4">
      <div className="flex items-center justify-between bg-white/40 backdrop-blur-xl p-2 md:p-4 rounded-[1.5rem] md:rounded-[2rem] border-2 md:border-4 border-white/60 shadow-[0_10px_30px_rgba(0,0,0,0.1)] gap-2">
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          <motion.div
            whileHover={{ scale: 1.1, rotate: 5 }}
            className={`${avatar.color} w-10 h-10 md:w-16 md:h-16 shrink-0 rounded-xl md:rounded-3xl flex items-center justify-center text-xl md:text-3xl shadow-lg border-2 md:border-4 border-white relative overflow-hidden`}
          >
            <div className="shine" />
            {avatar.image}
          </motion.div>
          <div className="min-w-0">
            <h2 className="text-sm md:text-xl font-black text-white drop-shadow-md tracking-tight truncate">Level {level.id}</h2>
