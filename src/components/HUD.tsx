...
const HUD: React.FC<HUDProps> = ({ title, score, targetScore, timeLeft, level, avatar }) => {
  ...
  return (
    <div className="w-full shrink-0 px-1 py-1 md:px-2 flex flex-col gap-2 md:gap-3">
      <div className="flex items-center justify-between bg-white/40 backdrop-blur-xl p-2 md:p-4 rounded-[1.1rem] md:rounded-[2rem] border-2 md:border-4 border-white/60 shadow-[0_10px_30px_rgba(0,0,0,0.1)] gap-2">
        ...
        <div className="flex flex-col items-end gap-0.5 md:gap-1">
          <div className="flex items-center gap-1 md:gap-2 bg-black/10 px-2 md:px-4 py-1 rounded-xl md:rounded-2xl border border-white/20">
            ...
            <span className={`text-xs md:text-xl font-black ${timeLeft < 10 ? "text-red-400" : "text-white"}`}>
              {timeLeft}s
            </span>
          </div>
          <motion.div
            ...
            className="text-lg md:text-4xl font-black text-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.3)]"
          >
            {score}
          </motion.div>
        </div>
      ...
      <div className="relative w-full h-4 md:h-10 bg-black/10 rounded-2xl md:rounded-3xl p-1 md:p-1.5 border-2 md:border-4 border-white/40 shadow-inner overflow-hidden">
