      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-3 backdrop-blur-md md:p-4">
          <motion.div
            ...
            className="app-modal-panel licensed-game-card-dark relative flex w-full max-w-xl flex-col overflow-hidden rounded-[1.75rem] p-3.5 shadow-[0_0_100px_rgba(0,0,0,0.35)] backdrop-blur-xl md:max-w-2xl md:rounded-[3rem] md:p-8"
          >
            <button
              onClick={onClose}
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <HUDBar eyebrow="Adventure tasks" title="Daily Quests" className="w-full justify-center text-center" />
              ...
              <div className="flex w-full flex-col gap-2.5">
                {quests.slice(0, 3).map((quest) => {
                  ...
                  return (
                    <div
                      key={quest.id}
                      className={`licensed-game-card flex flex-col gap-2.5 rounded-[1.05rem] p-3 text-white transition-all md:rounded-3xl md:p-5 ${quest.isClaimed ? 'grayscale opacity-50' : ''} ${isCompleted && !quest.isClaimed ? 'ring-2 ring-emerald-300/45' : ''}`}
                    >
