      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-md">
          <motion.div
            ...
            className="app-modal-panel casual-modal-panel relative flex w-full max-w-2xl flex-col overflow-hidden rounded-[2rem] border-4 border-white/20 bg-gradient-to-b from-indigo-600 to-purple-700 p-4 shadow-[0_0_100px_rgba(79,70,229,0.4)] md:rounded-[3rem] md:p-8"
          >
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              <div className="mb-4 flex h-18 w-18 rotate-2 items-center justify-center overflow-hidden rounded-[1.25rem] bg-white/10 p-2 shadow-2xl md:mb-6 md:h-24 md:w-24 md:rounded-3xl">
                <img src={rewardChest} className="h-full w-full object-contain" alt="reward chest" />
              </div>

              <h2 className="text-2xl font-black tracking-tight text-white drop-shadow-lg md:text-6xl">
                DAILY REWARDS
              </h2>
              <p className="mb-5 text-[10px] font-bold uppercase tracking-[0.22em] text-indigo-200 md:mb-8 md:text-sm md:tracking-widest">
                Login every day to earn bonuses
              </p>

              <div className="mb-5 grid w-full grid-cols-4 gap-2 md:mb-8 md:grid-cols-7 md:gap-3">
                {DAILY_REWARDS.map((day, idx) => {
                  ...
                  return (
                    <div
                      key={day.day}
                      className={`relative flex flex-col items-center gap-1 rounded-[1rem] border-2 p-2 transition-all md:rounded-2xl md:p-3 ${isCurrent ? 'z-10 scale-105 border-yellow-200 bg-yellow-400 shadow-xl' : 'border-white/10 bg-white/10'} ${isPast ? 'grayscale opacity-50' : ''}`}
                    >
              ...
              <div className="mb-5 w-full rounded-[1.3rem] border border-white/10 bg-black/20 p-4 backdrop-blur-md md:mb-8 md:rounded-3xl md:p-6">
                <div className="flex items-center justify-between gap-4">
                  <div className="text-left">
                    <div className="text-[10px] font-black uppercase tracking-[0.22em] text-white/60 md:text-xs md:tracking-widest">Current Streak</div>
                    <div className="text-2xl font-black text-white md:text-3xl">{streak} DAYS</div>
                  </div>
              ...
              <motion.button
                ...
                className={`w-full rounded-[1.2rem] py-4 text-base font-black transition-all md:rounded-3xl md:py-6 md:text-2xl ${claimedToday
