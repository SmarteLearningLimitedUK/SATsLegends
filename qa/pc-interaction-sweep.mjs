...
const REPORT_PATH = path.join(ROOT, 'reports', 'interaction-sweep-pc.md');
...
const parseTable = (sectionTitle) => {
  if (!fs.existsSync(DOC_PATH)) {
    throw new Error(`ENVIRONMENT_TRACKING.md not found: ${DOC_PATH}`);
  }
  const raw = fs.readFileSync(DOC_PATH, 'utf-8');
  const start = raw.indexOf(sectionTitle);
  if (start === -1) throw new Error(`Log section not found: ${sectionTitle}`);
...
const parsePcMinigames = () => parseTable('## 5) PC Minigame Verification Log');
const parseSmartphoneMinigames = () => parseTable('## 4) Smartphone Minigame Verification Log');
...
const attemptInteraction = async (page) => {
  ...
  try {
    ...
    let didPlayfieldClick = false;
    if (playfieldVisible) {
      const box = await playfield.boundingBox();
      if (box) {
        await page.mouse.click(box.x + box.width / 2, box.y + box.height / 2);
        didPlayfieldClick = true;
      }
    } else {
    ...
    const clickablePoint = await page.evaluate(() => {
      const root = document.querySelector('.structured-playfield-frame') || document.querySelector('main') || document.body;
      const candidates = Array.from(root.querySelectorAll('button:not([disabled]), [role="button"]:not([aria-disabled="true"])'));
      for (const el of candidates) {
        const hudAncestor = el.closest('[data-unified-minigame-hud="true"]');
        if (hudAncestor) continue;
...
const run = async () => {
  ...
  const source = (process.env.QA_SOURCE || 'pc').toLowerCase();
  const games = source === 'smartphone' ? parseSmartphoneMinigames() : parsePcMinigames();
  ...
  for (const entry of selectedGames) {
    const screenshotPath = path.join(OUTPUT_DIR, `qa-pc-interaction-${safeName(entry.minigame)}.png`);
    const gameResult = {
      island: entry.island,
