...
const REPORT_PATH = path.join(ROOT, 'reports', 'interaction-sweep-smartphone.md');
...
const parseSmartphoneMinigames = () => {
  if (!fs.existsSync(DOC_PATH)) {
    throw new Error(`ENVIRONMENT_TRACKING.md not found: ${DOC_PATH}`);
  }
  const raw = fs.readFileSync(DOC_PATH, 'utf-8');
  const start = raw.indexOf('## 4) Smartphone Minigame Verification Log');
  if (start === -1) throw new Error('Smartphone log section not found.');
...
const run = async () => {
  ...
  const games = parseSmartphoneMinigames();
  ...
  for (const entry of selectedGames) {
    const label = `${entry.island} - ${entry.minigame}`;
    const screenshotPath = path.join(OUTPUT_DIR, `qa-phone-interaction-${safeName(entry.minigame)}.png`);
    const gameResult = {
      island: entry.island,
    ...
    console.log(`Interaction sweep: ${label} -> ${gameResult.status}`);
  }
