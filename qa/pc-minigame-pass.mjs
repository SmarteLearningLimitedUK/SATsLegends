...
const minigamePlan = [
  ...
  { island: 'Cursed Colosseum', games: ['Reasoning Quest', 'Multi-Step Marathon', 'Strategy Survival', 'Mixed Mastery', 'Timed Test Trials', 'Median Master'] },
];
...
const run = async () => {
  ...
  for (const block of minigamePlan) {
    for (const game of block.games) {
      const outputName = `qa-pc-${safeName(game)}.png`;
  ...
  fs.writeFileSync(
    `${outputDir}/qa-pc-minigame-report.json`,
    JSON.stringify({ url, viewport: { width: 1440, height: 900 }, results }, null, 2),
  );
