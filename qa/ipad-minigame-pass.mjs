...
const minigamePlan = [
  ...
  { island: 'Data Desert', games: ['Mean Machine', 'Mode Miner', 'Graph Grabber', 'Line Graph Lab', 'Data Detective'] },
...
const run = async () => {
  ...
  for (const block of minigamePlan) {
    for (const game of block.games) {
      const outputName = `qa-ipad-${safeName(game)}.png`;
  ...
  fs.writeFileSync(
    `${outputDir}/qa-ipad-minigame-report.json`,
    JSON.stringify({ url, viewport: { width: 768, height: 1024 }, results }, null, 2),
  );
