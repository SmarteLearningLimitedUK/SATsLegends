...
const outputPath = 'D:/BrainZilla/SATsLegends/qa/qa-phone-data-detective.png';
...
const run = async () => {
  ...
  await openMap(page);
  await clickIfVisible(page.getByRole('button', { name: /data desert/i }), 2500);
  await page.waitForTimeout(600);
  await clickIfVisible(page.getByRole('button', { name: /explore island/i }), 1500);
  await page.waitForTimeout(900);

  const groupButton = page.locator('button[aria-expanded]', { hasText: /data detective/i }).first();
