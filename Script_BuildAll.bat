...
powershell -NoProfile -ExecutionPolicy Bypass -File "%UPLOAD_SCRIPT%" -LocalRoot "%SCRIPT_DIR%dist" -FtpHost "%FTP_HOST%" -Port "%FTP_PORT%" -Username "%FTP_USERNAME%" -Password "%FTP_PASSWORD%" -RemoteBaseDir "%FTP_REMOTE_DIR%" -UseSsl "%FTP_USE_SSL%" -AllowInsecureCertificate "%FTP_ALLOW_INSECURE_CERT%"
if errorlevel 1 (
